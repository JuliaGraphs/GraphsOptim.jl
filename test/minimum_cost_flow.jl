...
for n1 in 1:4
    @test sum(flow[n1, nout] for nout in outneighbors(g, n1)) ≈
        sum(flow[nin, n1] for nin in inneighbors(g, n1))
end

#--- Same test without edge_demands & positive cost should result in null flow
flow = minimum_cost_flow(
    g, node_demand, capacity, cost; optimizer=o, source_nodes=[5], sink_nodes=[6]
)
...
costs = ones(6, 6)
flow = minimum_cost_flow(
    g, spzeros(Graphs.nv(g)), capacity, costs; optimizer=o, edge_demand=demand
)
