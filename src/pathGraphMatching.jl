...
while(λ < 1.0)
    ...
    while abs(fλNormalizedFinal(p_new,λ_new,G,H)-fλNormalizedFinal(p_opt,λ,G,H)) < ϵ_λ_f   &&   p_change_normalized < ϵ_λ_p   &&   λ_new < one(Float64)
        dλ = 2*dλ
    ...
    while (abs(fλNormalizedFinal(p_new,λ_new,G,H)-fλNormalizedFinal(p_opt,λ,G,H)) > ϵ_λ_f   ||   p_change_normalized > ϵ_λ_p)   &&   dλ > dλ_min
        dλ = max(dλ/2,dλ_min)
        λ_new = min(λ + dλ, one(Float64))
        verbose && println("   dλ = ", dλ)
