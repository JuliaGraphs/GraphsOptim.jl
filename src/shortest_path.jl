...
function shortest_path!(
	...
	# Exit from source
	@constraint(model, sum(traversed[(source, next_v)] for next_v in outneighbors(dg, source)) - sum(traversed[(prev_v, source)] for prev_v in inneighbors(dg, source)) == 1)
	# Arrive at destination
	@constraint(model, sum(traversed[(target, next_v)] for next_v in outneighbors(dg, target)) + sum(traversed[(prev_v, target)] for prev_v in inneighbors(dg, target)) == 1)
	# Go out from a vertex is only possible by going in first
	for u in vertices(dg)
		if u in (source, target)
			continue
		end
		@constraint(model, sum(traversed[(u, next_v)] for next_v in outneighbors(dg, u)) - sum(traversed[(prev_v, u)] for prev_v in inneighbors(dg, u)) == 0)
	end

	@objective(model, Min, sum(edge_cost[u, v] * traversed[(u, v)] for (u, v) in edge_tuples))
